...
const BRIDGE_IDENTIFIERS = {
  BUNDLE: {
    bridgeType: 'bridge-cli-bundle',
    bridgeFileType: 'bridge-cli',
    bridgeFileNameType: 'bridge-cli-bundle'
  }
} as const
...
describe('BridgeCliBundle', () => {
  ...
  describe('Basic Properties', () => {
    testBridgeIdentifiers(() => bridgeCliBundle, BRIDGE_IDENTIFIERS.BUNDLE)
  })
  ...
  describe('verifyRegexCheck', () => {
    test.each([
      {
        url: 'https://example.com/bridge-cli-bundle-2.1.0-linux64.zip',
        expectedVersion: '2.1.0',
        shouldMatch: true
      },
      {
        url: 'https://example.com/bridge-cli-bundle-1.9.5-win64.zip',
        expectedVersion: '1.9.5',
        shouldMatch: true
      },
      {
        url: 'https://example.com/bridge-cli-bundle-3.0.0-macosx.zip',
      ...
      {
        url: 'https://example.com/wrong-prefix-2.1.0-linux64.zip',
        expectedVersion: null,
        shouldMatch: false
      }
    ])('should handle URL: $url', ({url, expectedVersion, shouldMatch}) => {
      const result = bridgeCliBundle.verifyRegexCheck(url)

      if (shouldMatch) {
        expect(result).toBeTruthy()
        expect(result![1]).toBe(expectedVersion)
      } else {
